...
export default function Observacoes() {
  ...
  return (
    <FormLayout title="Observações adicionais" step={9}>
      <div className="space-y-6">
        <div className="mt-2">
          <h3 className="text-lg font-medium mb-3">Selecione as observações:</h3>
          <div className="space-y-3">
            {OBSERVACOES_PREDEFINIDAS.map((item) => (
              <SelectionButton key={item} selected={data.observacoes.includes(item)} onClick={() => toggleItem(item)}>
                {item}
              </SelectionButton>
            ))}
          </div>
        ...
        <SelectedItemsList
          items={data.observacoes.filter((item) => !OBSERVACOES_PREDEFINIDAS.includes(item))}
          onRemove={removeObservacao}
        />

        <CustomItemInput onAdd={addObservacao} placeholder="Digite uma observação personalizada" />

        <NavigationButtons onBack={() => router.push("/form/preco")} onNext={handleNext} />
      </div>
    </FormLayout>
  )
}
