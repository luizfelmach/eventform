...
export function generateWhatsAppMessage(data: FormData): string {
  ...
  if (data.churrasco.selecionado) {
    message += `*Churrasco:*\n`;
    if (data.churrasco.itens.length > 0) {
      data.churrasco.itens.forEach((item) => {
        message += `- ${item}\n`;
      });
    } else {
      message += `- Não especificado\n`;
  ...
  if (data.coffeeBreak.selecionado) {
    message += `*Coffee Break:*\n`;
    if (data.coffeeBreak.itens.length > 0) {
      data.coffeeBreak.itens.forEach((item) => {
        message += `- ${item}\n`;
      });
    } else {
      message += `- Não especificado\n`;
